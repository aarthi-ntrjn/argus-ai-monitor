...
export async function startServer() {
  ...
  process.on('SIGTERM', async () => { telemetryService.sendEvent('app_ended'); slackListener?.shutdown(); slackNotifier?.shutdown(); monitor?.stop(); await app.close(); process.exit(0); });
  process.on('SIGINT', async () => { telemetryService.sendEvent('app_ended'); slackListener?.shutdown(); slackNotifier?.shutdown(); monitor?.stop(); await app.close(); process.exit(0); });
