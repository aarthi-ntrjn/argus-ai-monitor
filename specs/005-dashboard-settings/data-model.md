...
**Validation rules:**
- If the value read from `localStorage` cannot be parsed as valid JSON, fall back to the default `{ hideEndedSessions: false, hideReposWithNoActiveSessions: false }`.
...
**Filter independence**: `hideReposWithNoActiveSessions` evaluates against **all** sessions for a repo (not just the visible ones after `hideEndedSessions` filtering), so the two filters do not compound in unexpected ways.
...
```
DashboardSettings.hideEndedSessions:
  false  [user toggles on]>  true  [user toggles off]>  false
