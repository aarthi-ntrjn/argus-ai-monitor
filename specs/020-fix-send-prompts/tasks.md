...
- [x] T029 (redesigned) PTY launch creates a duplicate read-only detected session because the launcher immediately creates a DB session with a temp UUID, and the detector creates a second session when Claude's first hook fires. Fix: pending/claim pattern in PtyRegistry — `registerPending()` holds the WebSocket without creating a DB session; `claimForSession(claudeId, repoPath)` promotes it when the first hook fires; `handleHookPayload` claims first and creates ONE session with `launchMode='pty'` using Claude's real session ID. Launcher's close handler uses `getClaimedId(tempId)` to translate back. Files: `pty-registry.ts`, `launcher.ts`, `claude-code-detector.ts`.
