...
- [ ] T014 [US1] In `TeamsIntegrationService.onSessionCreated`: call `TeamsApiClient.createThread` with opening message containing session metadata (name, owner identity `ownerTeamsUserId`, start time per FR-011); persist `TeamsThread` record; log structured event `teams.thread.created`
...
- [ ] T017 [US1] Wire `TeamsIntegrationService` into `backend/src/server.ts`: instantiate after `SessionMonitor`; subscribe to `monitor.on('session.created')`, `monitor.on('session.updated')` (for output), `monitor.on('session.ended')`
