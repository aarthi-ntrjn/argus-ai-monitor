...
### Addendum: Bug — Claude sessions launched via "Launch with Argus" not detected on Windows

- [ ] T126 Fix `backend/src/services/claude-code-detector.ts` `HOOK_COMMAND`: the hook command injected into `~/.claude/settings.json` uses `2>/dev/null || true` which is Unix shell syntax. On Windows, cmd.exe (used by Claude Code to execute hooks) treats `2>/dev/null` as a redirect to a non-existent `dev\null` path and fails before curl executes, so the `SessionStart` hook POST never reaches Argus and the session is never detected. Fix: make `HOOK_COMMAND` platform-aware by importing `platform` from `os` and computing two variants: Unix keeps `2>/dev/null || true`; Windows uses `2>nul` (the cmd.exe null device) and omits `|| true` (not a Windows command). Also update `removeAllHooks()` and `hasHook()` to match against both variants so re-running on Windows correctly cleans up any previously injected Unix-style hook command.
